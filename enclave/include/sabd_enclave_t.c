...
static sgx_status_t SGX_CDECL sgx_sgxsd_enclave_test_ocall(void* pms)
{
	sgx_status_t status = SGX_SUCCESS;
...
SGX_EXTERNC const struct {
	...
	8,
	{
		...
		{(void*)(uintptr_t)sgx_sgxsd_enclave_test_ocall, 0},
	}
};

SGX_EXTERNC const struct {
	size_t nr_ocall;
	uint8_t entry_table[2][8];
} g_dyn_entry_table = {
	2,
	{
		{0, 0, 0, 0, 0, 0, 0, 0, },
...
sgx_status_t SGX_CDECL sgxsd_ocall_print_pt(void* pt)
{
	sgx_status_t status = SGX_SUCCESS;

	ms_sgxsd_ocall_print_pt_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_sgxsd_ocall_print_pt_t);
	void *__tmp = NULL;
	...
	ms = (ms_sgxsd_ocall_print_pt_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_sgxsd_ocall_print_pt_t));

	ms->ms_pt = SGX_CAST(void*, pt);
